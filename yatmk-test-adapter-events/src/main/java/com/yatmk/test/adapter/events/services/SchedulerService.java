...
public class SchedulerService implements LocalScheduler {
	...
	private Class<? extends Job> getJobClass(String jobName) {
		return Optional.ofNullable(jobName).map(quartzJobs::get).map(QuartzJobBean::getClass).orElseThrow(() -> new ServerSideException("No job found with name: " + jobName));
	}
	...
	private <T> JobDetail buildJobDetail(SchedulerParams<T> schedulerParams) {
		...
		return JobBuilder.newJob(jobClass).withIdentity(schedulerParams.getJobId(), schedulerParams.getGroup()).withDescription(schedulerParams.getDescription()).usingJobData(jobDataMap).storeDurably().build();
	}
	...
	private <T> Trigger buildJobTrigger(JobDetail jobDetail, SchedulerParams<T> schedulerParams) {
		return TriggerBuilder.newTrigger().forJob(jobDetail).withIdentity(schedulerParams.getJobId(), schedulerParams.getGroup()).withDescription(schedulerParams.getDescription()).startAt(Date.from(schedulerParams.getStartAt().toInstant())).withSchedule(getSchedule(schedulerParams.isReccurent(), schedulerParams.getCron())).build();
	}
