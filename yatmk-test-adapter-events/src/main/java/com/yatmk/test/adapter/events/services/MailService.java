...
public class MailService {
	...
	private MimeMessage addDest(MailEvent mailEvent, MimeMessage message) {
		try {
			List<String> to = Optional.ofNullable(mailEvent.getTo()).orElseGet(Collections::emptyList).stream().filter(Objects::nonNull).filter(s -> !s.isEmpty()).distinct().collect(Collectors.toList());

			for ( String dest : to ) {
				message.addRecipients(RecipientType.TO, dest);
			}

			List<String> cc = Optional.ofNullable(mailEvent.getCopy()).orElseGet(Collections::emptyList).stream().filter(Objects::nonNull).filter(s -> !s.isEmpty()).distinct().collect(Collectors.toList());
	...
	private Long getAttachmentsSize(MailEvent mailEvent) {
		return Optional.ofNullable(mailEvent).map(MailEvent::getAttachments).orElseGet(Collections::emptyList).stream().filter(Objects::nonNull).map(MailFile::getFileSize).reduce(0l, (a, b) -> a + b);
	}
	...
	private MimeMessage sendFiles(MailEvent mailEvent, Multipart multipart, MimeMessage message) {
		try {
			...
			Optional.ofNullable(mailEvent).map(MailEvent::getAttachments).orElseGet(Collections::emptyList).stream().filter(Objects::nonNull).forEach(e -> sendFile(e, multipart));
