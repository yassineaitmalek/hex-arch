...
public class ExcelReaderService {
	...
	public List<ExcelSheetData> readExcel(InputStream in, ExcelType ext, boolean isHeader) {
		try ( Workbook workbook = ExcelType.loadWorkbook(in, ext) ) {
			...
			return IntStream.range(0, workbook.getNumberOfSheets()).mapToObj(workbook::getSheetAt).map(e -> getSheetData(e, index, isHeader)).collect(Collectors.toList());
		} catch ( Exception e ) {
	...
	public int maxColumns(Sheet sheet) {
		return IntStream.range(0, sheet.getPhysicalNumberOfRows()).mapToObj(sheet::getRow).filter(Objects::nonNull).map(Row::getPhysicalNumberOfCells).filter(Objects::nonNull).max(Integer::compareTo).filter(Objects::nonNull).orElseGet(() -> 0);
	}

	public ExcelSheetData getSheetData(Sheet sheet, AtomicInteger index, boolean isHeader) {
		int maxColumns = maxColumns(sheet);
		List<List<String>> rows = IntStream.range((isHeader) ? 1 : 0, sheet.getPhysicalNumberOfRows()).mapToObj(sheet::getRow).map(row -> readLine(row, maxColumns)).collect(Collectors.toList());

		List<String> header = IntStream.range(0, (isHeader) ? 1 : 0).mapToObj(sheet::getRow).map(row -> readLine(row, maxColumns)).findFirst().orElseGet(Collections::emptyList);

		return ExcelSheetData.builder().sheetName(sheet.getSheetName()).index(index.getAndIncrement()).header(header).data(rows).nbRows(rows.size()).nbColumns(maxColumns).build();
	}
	...
	public List<String> readLine(Row row, int maxColumn) {
		return IntStream.range(0, maxColumn).mapToObj(index -> readLineImpl(row, index)).collect(Collectors.toList());
	}
	...
	public String checkField(String field) {
		return Optional.ofNullable(field).map(String::trim).filter(e -> !e.isEmpty()).orElseGet(() -> null);
	}
