...
public abstract class SimpleExcelWriter<T> implements ExcelSheetWriter {
	...
	private Sheet createLines(Sheet sheet) {
		...
		Optional.ofNullable(getLines()).orElseGet(Collections::emptyList).forEach(line -> {
	...
	private Sheet createHeader(Workbook workbook, Sheet sheet) {
		...
		Optional.ofNullable(getHeader()).orElseGet(Collections::emptyList).stream().forEach(e -> addCaption(sheet, col, row, e, style));
