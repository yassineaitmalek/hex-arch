...
public class ExcelSheetData {
	...
	public <T> List<T> parse(SimpleExcelParser<T> parser) {
		return Optional.ofNullable(data).orElseGet(Collections::emptyList).stream().map(parser::parse).filter(Optional::isPresent).map(Optional::get).collect(Collectors.toList());
	}
