package com.yatmk.test.adapter.output.persistence.config;
...
@EnableJpaRepositories(
        entityManagerFactoryRef = LocalDB.EMF, transactionManagerRef = LocalDB.TM, repositoryFactoryBeanClass = EnversRevisionRepositoryFactoryBean.class, basePackages = {LocalDB.BASE_REPO}
)
public class LocalDB {
	...
	public Map<String, String> getJpaProperties() {
		...
		properties.put(
		        "hibernate.hbm2ddl.auto", env.getProperty(String.format("%s.spring.jpa.hibernate.ddl-auto", DB))
		);
	...
	public LocalContainerEntityManagerFactoryBean getEntityManagerFactory(
	                                                                      EntityManagerFactoryBuilder builder, @Qualifier( DS ) DataSource dataSource, @Qualifier( JPA ) Map<String, String> jpaProperties
	) {
	...
	public PlatformTransactionManager getTransactionManager(
		...
		return Optional.ofNullable(entityManagerFactory).map(LocalContainerEntityManagerFactoryBean::getObject).map(JpaTransactionManager::new).orElseThrow(() -> new RuntimeException("Error initilizing transaction manager"));
	}
